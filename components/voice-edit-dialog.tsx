...
import { Edit, Mic, MicOff, AlertCircle } from "lucide-react"
import { useTextToSpeech } from "@/hooks/use-text-to-speech"
import { useSpeechRecognition } from "@/hooks/use-speech-recognition"
import { VoiceEditConversationManager } from "@/lib/voice-edit-conversation"
...
export function VoiceEditDialog({
  ...
  const [conversationManager] = useState(() => new VoiceEditConversationManager())
  ...
  useEffect(() => {
    if (open) {
      ...
      if (initialTranscript && initialTranscript.trim() && !initialProcessedRef.current) {
        ...
        // 초기 transcript가 없으면 기본 안내
        const initialMessage = "일정 수정을 시작합니다. 어떤 일정을 수정하시겠습니까?"
  ...
  useEffect(() => {
    if (transcript && !isProcessing && !isListening && transcript !== lastProcessedTranscript) {
      console.log('[Voice Edit Dialog] Processing transcript after recognition ended:', transcript)
  ...
  const handleUserInput = async (userInput: string) => {
    ...
    try {
      // AI로 입력 처리
      const result = await conversationManager.processVoiceInput(userInput, events)
      ...
      if (result.complete && result.targetEvent && result.updates) {
        // 일정 수정 완료
        const dateStr = result.updates.startDate
          ? formatDateKorean(result.updates.startDate)
          : formatDateKorean(result.targetEvent.startDate)
        const timeStr = result.updates.startDate
          ? result.updates.startDate.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })
          : result.targetEvent.startDate.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })
        const title = result.updates.title || result.targetEvent.title

        const completeMessage = `${title} 일정을 ${dateStr} ${timeStr}으로 수정했습니다.`
        setConversationLog(prev => [...prev, { role: 'assistant', text: completeMessage }])
        speak(completeMessage)

        // 일정 업데이트
        setTimeout(() => {
          onEventUpdate(result.targetEvent!.id, result.updates!)
          onOpenChange(false)
        }, 3000)
